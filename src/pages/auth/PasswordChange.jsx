      ...
      <Box sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', zIndex: 2 }}>
        <Card ref={cardRef} elevation={8} sx={{
          ...
          <Button variant="text" size="small" sx={{ color: '#a259ff', textTransform: 'none', fontWeight: 600, minWidth: 0, p: 0, mb: 1 }} onClick={() => setMode(mode === 'change' ? 'forgot' : 'change')}>
            {mode === 'change' ? 'Don\'t remember password? Use Email/Phone instead!' : 'Back to change password'}
          </Button>
            ...
            {mode === 'change' ? (
              <>
                <TextField
                  ...
                  sx={{ mb: 0.5, background: 'transparent', borderRadius: 2, border: '1px solid #4a90e2', input: { color: '#333', fontSize: 14, py: 0.8 }, minHeight: 36, fontSize: 14, transition: 'all 0.3s ease', zIndex: 2, position: 'relative' }}
              ...
              <>
                ...
                {method === 'email' ? (
                  <TextField placeholder="Email" type="email" fullWidth required value={email} onChange={e => setEmail(e.target.value)} sx={{ mb: 1, background: 'transparent', borderRadius: 2, border: '1px solid #4a90e2', input: { color: '#333', fontSize: 14, py: 0.8 }, minHeight: 36, fontSize: 14, transition: 'all 0.3s ease', zIndex: 2, position: 'relative' }} InputLabelProps={{ shrink: false }} />
                ) : (
                  <TextField placeholder="Phone" type="tel" fullWidth required value={phone} onChange={e => setPhone(e.target.value)} sx={{ mb: 1, background: 'transparent', borderRadius: 2, border: '1px solid #4a90e2', input: { color: '#333', fontSize: 14, py: 0.8 }, minHeight: 36, fontSize: 14, transition: 'all 0.3s ease', zIndex: 2, position: 'relative' }} InputLabelProps={{ shrink: false }} />
                )}
                {!otpVerified && (
                  <Button variant="outlined" size="small" sx={{ mb: 1, color: '#a259ff', borderColor: '#a259ff', fontWeight: 600, borderRadius: 2, '&:hover': { bgcolor: '#a259ff', color: '#fff', borderColor: '#a259ff' } }} onClick={handleSendOtp} disabled={otpSent || !(method === 'email' ? email : phone)}>
                ...
                <Collapse in={!!changeError}><Alert severity="error" sx={{ mb: 1 }}>{changeError}</Alert></Collapse>
                <Collapse in={!!changeSuccess}><Alert severity="success" sx={{ mb: 1 }}>{changeSuccess}</Alert></Collapse>
                <TextField placeholder="New Password" type={showPassword ? 'text' : 'password'} fullWidth required value={password} onChange={e => setPassword(e.target.value)} InputProps={{ endAdornment: (<InputAdornment position="end" sx={{ mr: 1 }}><IconButton onClick={() => setShowPassword(p => !p)} size="small" sx={{ color: '#666', '&:hover': { color: '#333', backgroundColor: 'rgba(0,0,0,0.05)' } }}>{showPassword ? <VisibilityOff fontSize="small" /> : <Visibility fontSize="small" />}</IconButton></InputAdornment>) }} sx={{ mb: 0.5, background: 'transparent', borderRadius: 2, border: '1px solid #4a90e2', input: { color: '#333', fontSize: 14, py: 0.8 }, minHeight: 36, fontSize: 14, transition: 'all 0.3s ease', zIndex: 2, position: 'relative' }} InputLabelProps={{ shrink: false }} disabled={!otpVerified} />
                {showPasswordChecks && !allValid && (
                  <Box sx={{ mb: 1, pl: 1 }}>
                    {passwordChecks.map((rule, i) => (
                      <Typography key={rule.label} variant="caption" sx={{ color: passwordStatus[i] ? 'green' : '#b71c1c', display: 'flex', alignItems: 'center', gap: 0.5 }}>
                ...
                <TextField placeholder="Confirm New Password" type={showConfirmPassword ? 'text' : 'password'} fullWidth required value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} InputProps={{ endAdornment: (<InputAdornment position="end" sx={{ mr: 1 }}><IconButton onClick={() => setShowConfirmPassword(p => !p)} size="small" sx={{ color: '#666', '&:hover': { color: '#333', backgroundColor: 'rgba(0,0,0,0.05)' } }}>{showConfirmPassword ? <VisibilityOff fontSize="small" /> : <Visibility fontSize="small" />}</IconButton></InputAdornment>) }} sx={{ mb: 0.5, background: 'transparent', borderRadius: 2, border: '1px solid #4a90e2', input: { color: '#333', fontSize: 14, py: 0.8 }, minHeight: 36, fontSize: 14, transition: 'all 0.3s ease', zIndex: 2, position: 'relative' }} InputLabelProps={{ shrink: false }} disabled={!otpVerified} />
                {confirmPassword && password && (
                  <Typography variant="caption" sx={{ color: passwordsMatch ? 'green' : '#b71c1c', pl: 1, fontWeight: 600 }}>
                    {passwordsMatch ? 'Passwords match' : 'Passwords do not match'}
                  </Typography>
